	...
	if(readXMLString(root, "race", strValue))
	{
		...
		if(tmpStrValue == "venom" || atoi(strValue.c_str()) == 1)
			...
			mType->race = RACE_ENERGY;
                ...
                else if(tmpStrValue == "grass" || atoi(strValue.c_str()) == 6)
                mType->race = RACE_GRASS;

                else if(tmpStrValue == "water" || atoi(strValue.c_str()) == 7)
                mType->race = RACE_WATER;
 
                else if(tmpStrValue == "bug" || atoi(strValue.c_str()) == 8)
                mType->race = RACE_BUG;

                else if(tmpStrValue == "normal" || atoi(strValue.c_str()) == 9)
                mType->race = RACE_NORMAL;

                else if(tmpStrValue == "poison" || atoi(strValue.c_str()) == 10)
                mType->race = RACE_POISON;
 
                else if(tmpStrValue == "electr" || atoi(strValue.c_str()) == 11)
                mType->race = RACE_ELECTR;

                else if(tmpStrValue == "ground" || atoi(strValue.c_str()) == 12)
                mType->race = RACE_GROUND;

                else if(tmpStrValue == "fairy" || atoi(strValue.c_str()) == 13)
                mType->race = RACE_FAIRY;

                else if(tmpStrValue == "fight" || atoi(strValue.c_str()) == 14)
                mType->race = RACE_FIGHT;

                else if(tmpStrValue == "psychc" || atoi(strValue.c_str()) == 15)
                mType->race = RACE_PSYCHC;

                else if(tmpStrValue == "ghost" || atoi(strValue.c_str()) == 16)
                mType->race = RACE_GHOST;

                else if(tmpStrValue == "rock" || atoi(strValue.c_str()) == 17)
                mType->race = RACE_ROCK;

                else if(tmpStrValue == "ice" || atoi(strValue.c_str()) == 18)
                mType->race = RACE_ICE;

                else if(tmpStrValue == "dragon" || atoi(strValue.c_str()) == 19)
                mType->race = RACE_DRAGON;

                else if(tmpStrValue == "steel" || atoi(strValue.c_str()) == 20)
