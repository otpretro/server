...
bool Game::playerBroadcastMessage(Player* player, SpeakClasses type, const std::string& text)
{
	...
	std::cout << "> " << player->getName() << " AUTO MENSAGEM: \"" << text << "\"." << std::endl;
	return true;
}
...
bool Game::playerOpenPrivateChannel(uint32_t playerId, std::string& receiver)
{
	...
	if(IOLoginData::getInstance()->playerExists(receiver))
		...
		player->sendCancel("O player com esse nome nao existe.");
	...
	uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	{
		player->sendTextMessage(MSG_INFO_DESCR, "desculpe, isso nao e possivel.");
		return false;
	}
	...
	for(std::map<Item*, uint32_t>::const_iterator it = tradeItems.begin(); it != tradeItems.end(); it++)
	{
		...
		{
			player->sendTextMessage(MSG_INFO_DESCR, "Este item ja esta no TRADE.");
			return false;
		}
	}
	...
	{
		player->sendTextMessage(MSG_INFO_DESCR, "Voce nao pode da TRADE com mais de 100 itens");
		return false;
	}
...
bool Game::internalStartTrade(Player* player, Player* tradePartner, Item* tradeItem)
{
	...
	{
		char buffer[100];
		sprintf(buffer, "%s quer negociar com voce", player->getName().c_str());
...
std::string Game::getTradeErrorDescription(ReturnValue ret, Item* item)
{
	...
	{
		ss << "Voce nao tem capacidade suficiente para transportar";
	...
	{
		ss << "Voce nao tem espaço suficiente para transportar";
		if(item->isStackable() && item->getItemCount() > 1)
			ss << " estes objectos.";
		else
			ss << " este objeto.";
		...
		ss << "A negociacao nao pode ser concluida.";
...
bool Game::internalCloseTrade(Player* player)
{
	...
	player->sendTextMessage(MSG_STATUS_SMALL, "Trade cancelado.");
	player->sendTradeClose();
	if(tradePartner)
	{
		...
		tradePartner->sendTextMessage(MSG_STATUS_SMALL, "Trade cancelado.");
		tradePartner->sendTradeClose();
	}
...
bool Game::playerLookAt(uint32_t playerId, const Position& pos, uint16_t spriteId, int16_t stackpos)
{
	...
	if(player->hasCustomFlag(PlayerCustomFlag_CanSeeCreatureDetails))
	{
		...
		{
			ss << std::endl << "Vida: [" << creature->getHealth() << " / " << creature->getMaxHealth() << "]";
			if(creature->getMaxMana() > 0)
				ss << ", Pokemons: [" << creature->getMana() << " / " << creature->getMaxMana() << "]";
...
bool Game::playerRequestAddVip(uint32_t playerId, const std::string& vipName)
{
	...
	{
		player->sendTextMessage(MSG_STATUS_SMALL, "Um jogador com esse nome nao existe.");
		return false;
	}

	if(specialVip && !player->hasFlag(PlayerFlag_SpecialVIP))
	{
		player->sendTextMessage(MSG_STATUS_SMALL, "Voce nao pode adicionar este jogador.");
		return false;
	}
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClasses type, const std::string& receiver, const std::string& text)
{
	...
	{
		char buffer[75];
		sprintf(buffer, "Voce ainda esta Muted por %d segundos.", muteTime);
	...
	const std::string& text)
{
	...
	{
		player->sendTextMessage(MSG_STATUS_SMALL, "Um jogador com este nome nao esta online.");
		return false;
	}
		...
		&& !player->hasFlag(PlayerFlag_CannotBeMuted))
	{
		char buffer[70];
		if(!canSee)
			sprintf(buffer, "Um jogador com este nome não está on-line.");
		else
			sprintf(buffer, "Desculpe, %s está a ignorar mensagens privadas.", toPlayer->getName().c_str());
	...
	char buffer[80];
	sprintf(buffer, "Mensagem enviada para %s.", toPlayer->getName().c_str());
	player->sendTextMessage(MSG_STATUS_SMALL, buffer);
	return true;
}
...
bool Game::playerReportRuleViolation(Player* player, const std::string& text)
{
	...
	{
		player->sendTextMessage(MSG_INFO_DESCR, "Relatorios de violacao de regra sao desativados.");
		return false;
	}
...
bool Game::playerContinueReport(Player* player, const std::string& text)
{
	...
	player->sendTextMessage(MSG_STATUS_SMALL, "Mensagem enviada para Gamemaster.");
	return true;
}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, int32_t manaChange)
{
	...
	if(manaChange > 0)
	{
			...
			(g_config.getBool(ConfigManager::SHOW_HEALING_DAMAGE_MONSTER) || !target->getMonster()))
		{
			...
			const SpectatorVec& list = getSpectators(targetPos);
			addAnimatedText(list, targetPos, TEXTCOLOR_RED, buffer);
		}
	}
	else
	{
...
bool Game::playerInviteToParty(uint32_t playerId, uint32_t invitedId)
{
	...
	{
		char buffer[90];
		sprintf(buffer, "%s ja esta em uma party.", invitedPlayer->getName().c_str());
...
bool Game::playerJoinParty(uint32_t playerId, uint32_t leaderId)
{
	...
	player->sendTextMessage(MSG_INFO_DESCR, "Voce ja esta em uma Party.");
	return false;
}
	...
	std::string comment, std::string statement, uint32_t statementId, bool ipBanishment)
{
	...
	{
		player->sendCancel("Voce nao tem autorizacao para esta acao.");
		return false;
	}
	...
	{
		char buffer[90];
		sprintf(buffer, "O comentario nao pode exceder o limite de %d por personagens.", commentSize);
	...
	{
		player->sendCancel("Um jogador com este nome não existe.");
		return false;
	}

	if(target->hasFlag(PlayerFlag_CannotBeBanned))
	{
		player->sendCancel("Você não tem autorização para esta ação.");
		return false;
	}
	...
	switch(action)
	{
		case ACTION_STATEMENT:
		{
			...
			{
				player->sendCancel("Declaracao ja foi relatado.");
				return false;
			}
		...
		case ACTION_BANREPORTFINAL:
		{
			...
			{
				account.warnings--;
				player->sendCancel("Conta foi Banida.");
				return false;
			}
		...
		case ACTION_DELETION:
		{
			...
			{
				account.warnings--;
				player->sendCancel("Conta atualmente esta BANIDA ou ja foi DELETADA.");
				return false;
			}
...
bool Game::broadcastMessage(const std::string& text, MessageClasses type)
{
	...
	std::cout << "> AUTO MENSAGEM: \"" << text << "\"." << std::endl;
	for(AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
...
void Game::showHotkeyUseMessage(Player* player, Item* item)
{
	...
	if(count == 1)
		sprintf(buffer, "Usando %s...", it.name.c_str());
	else
		sprintf(buffer, "Usando %d %s...", count, it.pluralName.c_str());
