...
bool ProtocolGame::parseFirstPacket(NetworkMessage& msg)
{
	...
	if(name.empty())
	{
		...
		{
			disconnectClient(0x14, "Conta Errada.");
			return false;
		}
	...
	{
		disconnectClient(0x14, "IP bloqueado por alguns minutos, muitas tentivas para entrar.");
		return false;
	}

	if(IOBan::getInstance()->isIpBanished(getIP()))
	{
		disconnectClient(0x14, "IP BANIDO!");
		return false;
	}
	...
	{
		ConnectionManager::getInstance()->addAttempt(getIP(), protocolId, false);
		disconnectClient(0x14, "Senha errada.");
		return false;
	}
			...
			default:
			{
				if(g_config.getBool(ConfigManager::BAN_UNKNOWN_BYTES))
				{
						...
						"Sending unknown packets to the server.", 0, player->getGUID()))
					{
						...
						player->sendTextMessage(MSG_INFO_DESCR, "Voce foi Banido.");
