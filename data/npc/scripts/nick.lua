...
if (msgcontains(msg, 'hi') and (focus == 0) and (getDistanceToCreature(cid) <= 4)) then
	...
	conv = 1
	talk_start = os.clock()
	selfSay("Hello, "..getCreatureName(cid).."! Eu posso mudar o nome do seu pokemon por 1 Ruby Coin...")
...
elseif (msgcontains(msg, "nick") or msgcontains(msg, "nickname")) and focus == cid and conv == 1 then
		...
		if getPlayerSlotItem(cid, 8).uid <= 0 then
			selfSay("Coloque seu pokemon no slot!")
			focus = 0
		return true
		end

	selfSay("Coloque seu pokemon no slot e diga qual nome deseja?")
	conv = 3
...
elseif msgcontains(msg, "aura") and focus == cid and conv == 1 then
     ...
     if not getItemAttribute(pb, "boost") or getItemAttribute(pb, "boost") < 50 then
        selfSay("Desculpe, mas seu pokemon nao esta +50!")
        focus = 0
     return true
     end  
     ...
     if #getCreatureSummons(cid) >= 1 then 
        selfSay("Volte seu pokemon para pokebola")
        focus = 0
     return true
     end       
...
elseif isInArray(auras, msg) and focus == cid and conv == 9 then
       ...
       selfSay("Tem certeza que deseja colocar "..msg.." no seu pokemon?")
       conv = 11 
...
elseif msgcontains(msg, "yes") and focus == cid and conv == 11 then        
     ...
     local pb = getPlayerSlotItem(cid, 8).uid
     if not getItemAttribute(pb, "boost") or getItemAttribute(pb, "boost") < 50 then
        selfSay("Desculpe, seu pokemon nao tem boost+50!")
        focus = 0
     return true
     end
     
     if #getCreatureSummons(cid) >= 1 then 
        selfSay("Volte o pokemon para pokebola!")
        focus = 0
     return true
     end   
...
elseif conv == 3 and focus == cid then
	...
	for a = 1, #table do
		if string.find(msg, table[a]) then
			selfSay("Desculpe, mas esse nome tem simbolos invalidos.")
		return true
		end
	end

	if string.len(msg) <= 1 or string.len(msg) >= 19 then
		selfSay("Desculpe, mas esse nome e muito longo!")
	return true
	end
	...
	selfSay("Voce tem certeza que quer mudar o "..pokename.." nome para \""..msge.."\"? Isso vai custar 25 black diamonds")
	conv = 5
	finalname = msge
...
elseif msgcontains(msg, "yes") and focus == cid and conv == 5 then
	...
	if doPlayerRemoveItem(cid,2152, 1) == false then
		selfSay("Voce nao tem Ruby Coin suficientes e preciso de 25.")
		focus = 0
		conv = 0
	return true
	end
	...
	selfSay("Parabens, agora seu pokemon tem um novo nome.")
