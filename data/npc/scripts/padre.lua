...
keywordHandler:addKeyword({'jobxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "I am a priest of the great pantheon."})
keywordHandler:addKeyword({'kingxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "King Tibianus is our benevolent sovereign."})
keywordHandler:addKeyword({'lifexs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "Life is a gift of the gods, honor life and don't destroy it."})
keywordHandler:addKeyword({'missionxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "It is my mission to spread knowledge about the gods."})
keywordHandler:addKeyword({'questxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "It is my mission to spread knowledge about the gods."})
keywordHandler:addKeyword({'namexs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "My name is Lynda. And the spirits tell me that you are |PLAYERNAME|."})
keywordHandler:addKeyword({'tibiaxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "The world of Tibia is the creation of the gods."})
keywordHandler:addKeyword({'timexs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "Now, it is "..getWorldTime()..""})
keywordHandler:addKeyword({'monstersxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "They are creatures of the gods of evil!"})
keywordHandler:addKeyword({'firesxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "Fire is one of the primal elemental forces, sometimes worshipped by tribal shamans."})
keywordHandler:addKeyword({'airsxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "Air is one of the primal elemental forces, sometimes worshipped by tribal shamans."})
...
function MageFirst(cid, message, keywords, parameters, node)
    ...
    if addon == -1 then
        ...
        if doPlayerRemoveItem(cid,2181,1) and doPlayerRemoveItem(cid,2182,1) and doPlayerRemoveItem(cid,2183,1) and doPlayerRemoveItem(cid,2185,1) and doPlayerRemoveItem(cid,2186,1) and doPlayerRemoveItem(cid,2187,1) and doPlayerRemoveItem(cid,2188,1) and doPlayerRemoveItem(cid,2189,1) and doPlayerRemoveItem(cid,2190,1) and doPlayerRemoveItem(cid,2191,1) and doPlayerRemoveItem(cid,5904,10) and doPlayerRemoveItem(cid,2193,20) and doPlayerRemoveItem(cid,5809,1) then
            ...
            doPlayerAddOutfit(cid, 1289, 1)
			elseif getPlayerSex(cid) == 0 then
            doPlayerAddOutfit(cid, 1290, 1)
...
node1 = keywordHandler:addKeyword({'mage wandxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To get mage wand you need give me wand of vortex,wand of dragonbreath,wand of decay,wand of cosmic energy, wand of inferno, snakebite rod, moonlight rod, necrotic rod, terra rod, hailstorm rod, 10 magic sulphurs, 20 ankhs and a soul stone. Do you have it with you?'})
node1:addChildKeyword({'yes'}, MageFirst, {})
node1:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Alright then. Come back when you got all neccessary items.', reset = true})

node3 = keywordHandler:addKeyword({'addonxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To get mage wand you need give me wand of vortex,wand of dragonbreath,wand of decay,wand of cosmic energy, wand of inferno, snakebite rod, moonlight rod, necrotic rod, terra rod, hailstorm rod, 10 magic sulphurs, 20 ankhs and a soul stone. Do you have it with you?'})
...
node2 = keywordHandler:addKeyword({'summoner wandxs'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To get summoner wand you need give me wand of vortex,wand of dragonbreath,wand of decay,wand of cosmic energy, wand of inferno, snakebite rod, moonlight rod, necrotic rod, terra rod, hailstorm rod, 10 magic sulphurs, 20 ankhs and a soul stone. Do you have it with you?'})
				...
				if getPlayerStorageValue(cid, 46000) == 1 then
					if getPlayerStorageValue(player, 46001) == getPlayerGUID(cid) then
						...
						for _, pid in ipairs({cid, player}) do
							doPlayerAddOutfitId(pid, 1290, 0)
							local v = getCreatureOutfit(pid)
							v.lookType = getPlayerSex(pid) == 0 and 1290 or 1289
						...
						local nc, np = getCreatureName(cid), getCreatureName(player)
                                                setPlayerStorageValue(cid,fmage, 1)
						doItemSetAttribute(doPlayerAddItem(cid, 7708, 1), 'description', np .. ' & ' .. nc .. ' forever - married on ' .. os.date('%b ' .. os.date('*t').day .. ', %Y') .. '.')
						doItemSetAttribute(doPlayerAddItem(player, 7708, 1), 'description', nc .. ' & ' .. np .. ' forever - married on ' .. os.date('%b ' .. os.date('*t').day .. ', %Y') .. '.')
...
function creatureSayCallback(cid, type, msg)
	...
	elseif not npcHandler:isFocused(cid) then
		return false
	elseif msgcontains(msg, 'bye') or msgcontains(msg, 'farewellxs') then
		npcHandler:say('Good bye, '..getCreatureName(cid)..'. May the Gods watch over you, my child!', cid)
		Topic[cid] = nil
		npcHandler:releaseFocus(cid)
	elseif msgcontains(msg, 'marriage') or msgcontains(msg, 'marryxs') or msgcontains(msg, 'ceremony') or msgcontains(msg, 'weddingxs') then
	...
	elseif Topic[cid] == 6 then
		...
		Topic[cid] = nil
	elseif msgcontains(msg, 'melunaxs') then
		if getPlayerPartner(cid) ~= 0 then
			if getPlayerSex(cid) == 1 then
	...
	elseif Topic[cid] == 7 then
		if msgcontains(msg, 'yes') then
			...
			doTeleportThing(cid, {x=1022, y=889, z=7})
			doSendMagicEffect({x=1022, y=890, z=7}, CONST_ME_HEARTS)
		else
			say('Hm, then not.', cid)
		end
		Topic[cid] = nil
	...
	elseif Topic[cid] == 9 then
		marry(cid, msg, true)
	elseif msgcontains(msg, 'ringxs') then
